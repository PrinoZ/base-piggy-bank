...
const PlanCard = ({ job, isTemplate = false, onCancel, isLoading, usdcBalance, refreshTrigger }: any) => {
    ...
    const handleShare = async (e: any) => {
        ...
        // 引导到具�?OG/Frame 预览的页面（主页�?/embed），让社交平台抓�?og-image
...
export default function App() {
  ...
  const handleRefresh = async (userAddr = address) => {
      ...
      // 余额现在�?useReadContract 自动管理，不需要手�?fetch
  ...
  const handleStartDCA = async () => {
    ...
    try {
      ...
      const nonce = crypto.randomUUID ? crypto.randomUUID() : `${now}-${Math.random()}`;
        ...
        // 即使 ERC20_ABI 用了 parseAbi，Ethers v6 也能完美兼容�?        const usdcContract = new ethers.Contract(USDC_ADDRESS, ERC20_ABI, signer);
        ...
        {activeTab === 'strategy' && (
          <div className="flex flex-col h-full">
            ...
            <div className="flex-none p-5 bg-white space-y-3">
              ...
              <div className="pt-1">
                {/* 这里的按钮逻辑现在只负责触�?handleStartDCA，如果没连接，函数内部会拦截 */}
