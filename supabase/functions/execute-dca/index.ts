...
Deno.serve(async (req) => {
  try {
    ...
    // 强制 Gas Limit，跳过预估检�?    const txOptions = {
    ...
    for (const job of jobs) {
      let txHash = null;
      let status = 'SUCCESS'; // 默认先设为成�?      let errorMessage = null;
      ...
      try {
        ...
        // === 2. 关键修改：等待链上确�?===
        // Base 链很快，通常 2-3 秒就能确�?        console.log("Waiting for receipt...");
        const receipt = await tx.wait();

        // === 3. 检查链上真实状�?===
        // status: 1 代表成功�? 代表失败(Revert)
        if (receipt.status === 0) {
        ...
        // 🛑 一旦报错（包括链上Revert），标记为失�?        status = 'FAILED'; 
        
        // 如果没有生成 Hash (比如发之前就挂了)，生成一个错误标�?        if (!txHash) {
      ...
      const { error: logError } = await supabase
        ...
        .insert({
            ...
            tx_hash: txHash,
            status: status, // 这里现在能正确记�?'FAILED' �?            created_at: new Date().toISOString()
        });
      ...
      if (status === 'SUCCESS') {
        ...
        // Upsert 更新排行�?        const { error: leaderboardError } = await supabase
          .from('leaderboard_table')
          .upsert({
